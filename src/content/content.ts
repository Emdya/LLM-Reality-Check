...
function getAIContent(): string {
  ...
  if (chatGPTMessages.length > 0) {
    const lastMessage = chatGPTMessages[chatGPTMessages.length - 1] as HTMLElement
  ...
  if (!content) {
    ...
    if (claudeMessages.length > 0) {
      const lastMessage = claudeMessages[claudeMessages.length - 1] as HTMLElement
      content = lastMessage.innerText || lastMessage.textContent || ""
    }
  }
...
function handleVerificationResults(results: any) {
  isVerifying = false

  if (results.hasIssues && results.falseStatements) {
    results.falseStatements.forEach((statement: any) => {
...
function highlightFalseText(text: string) {
...
function insertCorrectionPrompt(correction: string) {
  // ChatGPT input selector
  let inputElement = document.querySelector('#prompt-textarea, [data-id="root"] textarea') as HTMLTextAreaElement
  ...
  if (inputElement) {
    ...
    if (inputElement.tagName === "TEXTAREA") {
      ;(inputElement as HTMLTextAreaElement).value = correctionText
